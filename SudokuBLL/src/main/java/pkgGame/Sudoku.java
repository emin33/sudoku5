...
public class Sudoku extends LatinSquare implements Serializable {
	...
	private static int PossibleValuesMultiplier(HashMap<Integer, SudokuCell> cells) {
		...
		while (it.hasNext()) {
			...
			if (mult * SC.getLstRemainingValidValues().size() > Integer.MAX_VALUE) {
				return Integer.MAX_VALUE;
			} else {
				mult *= SC.getLstRemainingValidValues().size();
			}
		}
		return mult;
	}
	...
	private void RemoveCells() {
		...
		do {
			...
			int randRow = rand.nextInt(this.size);
			int randCol = rand.nextInt(this.size);
			this.getPuzzle()[randRow][randCol]] = 0;
			SetRemaingCells();
	...
	private void SetRemaingCells() {
		for(int row = 0; row < size; row++) {
			for (int col = 0; col < size; col++) {
				SudokuCell c1 = new SudokuCell(row, col);
				c1.setlstRemainingValidValues(getAllValidCellValues(col, row));
				cells.put(c1.hashCode(), c1);
			}
		}
	}
	
	public Sudoku(int size, eGameDifficulty e) throws Exception {
		this(size);
		this.eGameDifficulty = e;
		RemoveCells();
	}
